...
""")
        ...
        found_cycle = False
        for cycle in cycles:
            if ('aws_security_group.sg_a' in cycle and 
                'aws_security_group.sg_b' in cycle):
                found_cycle = True
                break
...
def test_file_paths_are_absolute():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert vpc_path.endswith(f"infra/network/resources.tf")
        assert subnet_path.endswith(f"infra/network/resources.tf")
        assert sg_path.endswith(f"infra/compute/resources.tf")
        assert instance_path.endswith(f"infra/compute/resources.tf")
        ...
        assert f"infra/network/resources.tf" in md_output
        assert f"infra/compute/resources.tf" in md_output
