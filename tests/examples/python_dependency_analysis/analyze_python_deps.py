...
def main():
    ...
    parser.add_argument("--repo-path", default=str(repo_root),
                        help=f"Path to the repository (default: {repo_root})")
    parser.add_argument("--output-dir", default=str(script_dir / "output"),
                        help="Directory to store output files")
    parser.add_argument("--format", choices=["json", "dot", "graphml", "adjacency"], default="dot",
    ...
    parser.add_argument("--llm-context", action="store_true",
                        help="Generate LLM-friendly context about the code structure")
    parser.add_argument("--module", 
                        help="Analyze dependencies for a specific module (e.g., 'kit.dependency_analyzer')")
    ...
    modules_by_imports = sorted(
        [(module, len(graph[module].get('dependencies', []))) 
    ...
    most_imported = sorted(
        [(module, count) for module, count in module_usage.items()],
        key=lambda x: x[1],
        reverse=True
    )
