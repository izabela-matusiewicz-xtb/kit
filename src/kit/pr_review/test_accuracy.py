...
class AccuracyTester:
    ...
    def __init__(self, config: ReviewConfig):
        self.config = config
        self.simple_reviewer = SimplePRReviewer(config)
    ...
    def test_pr_across_modes(self, pr_url: str) -> Dict[str, Any]:
        """Test a PR across all review modes and compare quality."""
        ...
        print("-" * 20)
        try:
            simple_review = self.simple_reviewer.review_pr_simple(pr_url)
            results['simple'] = {
                'review': simple_review,
                'success': True,
                'cost': self.simple_reviewer.cost_tracker.get_total_cost()
            }
    ...
    def validate_existing_review(self, pr_url: str, review_content: str) -> Dict[str, Any]:
        ...
        try:
            # Get PR data
            owner, repo, pr_number = self.simple_reviewer.parse_pr_url(pr_url)
            files = self.simple_reviewer.get_pr_files(owner, repo, pr_number)
            pr_diff = self.simple_reviewer.get_pr_diff(owner, repo, pr_number)
