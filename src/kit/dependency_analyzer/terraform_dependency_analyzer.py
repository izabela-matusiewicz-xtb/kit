...
class TerraformDependencyAnalyzer(DependencyAnalyzer):
    ...
    def _add_node(self, node_id: str, node_category: str, node_type: str, 
                 node_name: str, file_path: str):
        """
    ...
    def export_dependency_graph(self, output_format: str = 'json', output_path: Optional[str] = None) -> Union[Dict, str]:
        ...
        for node_id, data in self.dependency_graph.items():
            serializable_graph[node_id] = {
                ...
                'name': data['name'],
                'path': data['path'],
                'dependencies': list(data['dependencies']) if isinstance(data['dependencies'], set) else data['dependencies']
            }
    ...
    def get_resource_dependencies(self, resource_id: str, include_indirect: bool = False) -> List[str]:
        ...
        if include_indirect:
            ...
            return [
                dep for dep in self.dependency_graph[resource_id]['dependencies']
                if dep in self.dependency_graph
            ]
    ...
    def get_dependents(self, resource_id: str, include_indirect: bool = False) -> List[str]:
        ...
        direct_dependents = [
            node for node, data in self.dependency_graph.items()
            if resource_id in data['dependencies']
        ]
        ...
        def find_ancestors(node):
            parents = [
                parent for parent, data in self.dependency_graph.items()
    ...
    def get_resource_by_type(self, resource_type: str) -> List[str]:
        ...
        return [
            node_id for node_id, data in self.dependency_graph.items()
    ...
    def visualize_dependencies(self, output_path: str, format: str = 'png') -> str:
        ...
        dot = graphviz.Digraph(
            ...
            comment='Terraform Resource Dependencies',
            format=format,
            engine='dot'
        )
        dot.attr(rankdir='LR')
    ...
    def generate_llm_context(self, max_tokens: int = 4000, output_format: str = 'markdown', output_path: Optional[str] = None) -> str:
        ...
        key_resources = sorted(
            resource_importance.items(),
            key=lambda x: x[1],
            reverse=True
        )[:5]
        
        modules = [node_id for node_id, data in self.dependency_graph.items() 
                  if data.get('category') == 'module']
