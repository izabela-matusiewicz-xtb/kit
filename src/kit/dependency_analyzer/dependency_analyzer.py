...
class DependencyAnalyzer(ABC):
    ...
    def generate_llm_context(self, max_tokens: int = 4000, output_format: str = 'markdown', output_path: Optional[str] = None) -> str:
        ...
        internal_nodes = len([n for n, data in self.dependency_graph.items() 
        ...
        for node in self.dependency_graph:
            ...
            incoming = len([n for n in self.dependency_graph 
                          if node in self.dependency_graph[n].get('dependencies', [])])
    ...
    def get_for_language(cls, repository: 'Repository', language: str) -> 'DependencyAnalyzer':
        ...
        if language == 'python':
            ...
            raise ValueError(f"Unsupported language for dependency analysis: {language}. "
