...
|-----------------|-------------|--------|
| `/semantic-search` | Embedding-based search using vector databases (ChromaDB) to find semantically similar code chunks | Temporarily removed, coming soon |
| Enhanced symbol analysis | Improved cross-language symbol detection and relationship mapping | Planned |
| Code modification endpoints | API for applying changes to code with safety checks | Under consideration |
...
*   `501 Not Implemented`: A core component for the requested capability is missing from the server installation (e.g., an LLM SDK for `/summary` if it were optional, or `chromadb` for `/semantic-search`). Should be rare for default installs.
